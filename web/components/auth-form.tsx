...
export default function AuthForm({
  ...
  mode: "login" | "cadastro" | "recuperar" | "nova";
}) {
  ...
  const title = {
    ...
    cadastro: "Comece a organizar suas obras",
    recuperar: "Recuperar senha",
    nova: "Defina sua nova senha",
  }[mode];
  async function submit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if ((signup || mode === "nova") && password !== f.get("confirmar"))
      ...
      if (signup && !String(f.get("nome") || "").trim())
      ...
      if (signup) {
        const { data, error } = await supabase.auth.signUp({
          ...
          password,
          options: {
            emailRedirectTo: window.location.origin + "/obras",
            data: {
  ...
  return (
    <div className="auth-layout">
      <aside className="auth-aside">
        ...
        <div>
          <p className="eyebrow">DO PLANEJAMENTO À EXECUÇÃO</p>
          <h1>
            Sua obra.
            <br />
            Cada etapa
          ...
          <p>
            Reúna pessoas, organize o trabalho e acompanhe o que acontece no
            canteiro.
          </p>
          <div className="auth-lines">
            <span>01 &nbsp; Planeje com clareza</span>
            <span>02 &nbsp; Acompanhe sua equipe</span>
            <span>03 &nbsp; Registre cada avanço</span>
          </div>
        </div>
        <small>Gestão de obras, sem complicação.</small>
      </aside>
      <main className="auth-main">
        <div className="auth-card">
          <span className="pill">
            <ShieldCheck size={15} /> Seu espaço de gestão
          </span>
          <h2>{title}</h2>
          <p className="muted">
            {signup
            ...
            {mode !== "recuperar" && (
              <label>
                ...
                <input
                  ...
                  required
                />
                {mode !== "login" && <small>Mínimo de 8 caracteres.</small>}
              </label>
            )}
            {(signup || mode === "nova") && (
              <label>
